...
module.exports = message => {
	...
	var input = message_.content.substring(7)
		.split(' ')
	...
	db_.get(sql, [pokemon], function(err, row) {
		if (err) return console.log('uniq.message() - ' + err.message);
		if (row.count == 0) {
...
function callback_getData(pokemon, adjective){
	...
	{
		// evolve the first entry (oldest caught by a specific user)
		...
		db_.get(sql, [pokemon, message_.author.id], function(err, row) {
			...
			if (row === undefined) {
				...
				callback_checkEvolve(pokemon, row.dogtag, row.adjective);
			}
		});
	} else {
		// evolve the first entry (oldest caught by a specific user for given adjective)
		...
		db_.get(sql, [pokemon, message_.author.id, adjective], function(err, row) {
			...
			if (row === undefined) {
				...
				callback_checkEvolve(pokemon, row.dogtag, adjective);
			}
		});
	}
}

function callback_checkEvolve(pokemon, dogtag, adjective){
	var sql = `SELECT * FROM evolution_map WHERE pokemon_name = (?)`;
	db_.all(sql, [pokemon], function(err, rows) {
		if (err) return console.log('callback_checkEvolve() - ' + err.message);
		if (rows.length == 0) {
			callback_cantEvolve(pokemon);
		} else {
			callback_evolve(pokemon, dogtag, adjective, rows[Math.floor(Math.random() * rows.length)]);
		}
	});
}

function callback_evolve(pokemon, dogtag, adjective, evo){
	var sql = `UPDATE caught_pokemon SET pokemon_id = (?) WHERE dogtag = (?)`;
...
function callback_cantEvolve(pokemon){
	// message channel
	const embed = new RichEmbed()
		.setAuthor(message_.author.username, message_.author.avatarURL)
		.setTitle(pokemon)
		.setColor(0xFF0000)
		.setThumbnail('https://img.pokemondb.net/sprites/emerald/normal/' + pokemon.toLowerCase().replace(/[^a-zA-Z ]/g, '').replace(' ', '-') + '.png')
		.setDescription('Sorry *' + message_.author.username + '*, you cannot evolve your ' + pokemon + '...');
	message_.channel.send(embed);
}
